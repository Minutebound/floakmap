...
def get_facilities(
    category: Optional[str] = Query(None, description="Filter by category: parking|carwash|ev|auto"),
...
@router.get("/facilities/categories")
def get_categories():
    """Return all categories with their counts."""
    counts: Counter = Counter(f["properties"]["category"] for f in _FEATURES)
    return {
        "categories": [
            {"key": "parking", "label": "Parking Facilities", "color": "#2563eb", "count": counts.get("parking", 0)},
            {"key": "carwash",  "label": "Car Washes",          "color": "#0891b2", "count": counts.get("carwash",  0)},
            {"key": "ev",       "label": "EV Charging",          "color": "#16a34a", "count": counts.get("ev",       0)},
            {"key": "auto",     "label": "Auto Services",        "color": "#ea580c", "count": counts.get("auto",     0)},
        ]
    }
...
@router.get("/cities")
def get_cities():
    """Return distinct cities that have data."""
    seen: set[str] = set()
    cities = []
    for f in _FEATURES:
        p = f["properties"]
        ...
        if key not in seen:
            seen.add(key)
            cities.append({"city": p["city"], "state": p["state"], "label": key})
